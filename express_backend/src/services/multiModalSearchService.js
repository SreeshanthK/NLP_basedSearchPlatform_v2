...
class MultiModalSearchService {
    ...
    async mongodbSearch(mongoClient, query, filters, nlpAnalysis) {
        if (!mongoClient) {
            return [];
        }
        ...
        try {
            ...
            try {
                ...
                mongoResults = await collection.find(mongoQuery)
                    .limit(50)
                    .toArray();

                if (mongoResults.length < 20) {
                    const regexQuery = {
                        $or: [
                    ...
                    const regexResults = await collection.find(regexQuery)
                        .limit(30)
                        .toArray();

                    const existingIds = new Set(mongoResults.map(r => r._id.toString()));
                    const newResults = regexResults.filter(r => !existingIds.has(r._id.toString()));
                    mongoResults = [...mongoResults, ...newResults];
                }
            ...
            const resultsWithReviews = await this.populateReviews(mongoResults, db);
